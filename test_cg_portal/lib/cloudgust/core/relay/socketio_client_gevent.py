...
class SocketIO(object):
    ...
    def __init__(self, host, port, secure=False, headers=None, proxies=None):
        ...
        gevent.spawn(self._rhythmicThread.run)
        gevent.sleep(0)

        self._listenerThread = _ListenerThread(
            self._socketIO,
            self._namespaceByPath)
        gevent.spawn(self._listenerThread.run)
        gevent.sleep(0)
    ...
    def run(self):
        while not self.done.is_set():
            ...
            try:
                ...
                continue
            gevent.spawn(delegate, packetID, namespace._get_eventCallback, data)
            gevent.sleep(0)
...
class _SocketIO(object):
    ...
    def send_heartbeat(self):
        try:
            ...
            # this is the modification done, only when heartbeat sending is failed
            # final error is thrown, and not at connection lost
            self.connection.connected = False
            self.connection.error = SocketIOConnectionError('connection lost')
    ...
    def recv_packet(self):
        try:
            ...
            text = 'Lost connection will retry in less than 60 sec '
            raise SocketIOConnectionError(text)
        try:
